...
app.get("/random", (req, res) => {
  const numRandom = Math.floor(Math.random() * jokes.length);
  const numJoke = jokes[numRandom];
  res.json(numJoke);
});
...
app.patch("/jokes/:id", (req, res) => {
  ...
  console.log(currentJoke.jokeText);
  ...
  const currentIndex = jokes.findIndex((joke) => joke.id === id);
  jokes[currentIndex] = updateJoke;
  res.json(updateJoke);
});
...
app.delete("/jokes", (req, res) => {
  jokes = [];
  res.status(200).json({ message: "Jokes deleted successfully." });
});
app.listen(port, () => {
